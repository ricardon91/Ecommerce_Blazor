...
namespace RazorShop.Web.Services
{
	public class GerenciaProdutosLocalStorageService : IGerenciaProdutosLocalStorageService
	{
		...
		{
			return await this.localStorageService.GetItemAsync<IEnumerable<ProdutoDto>>(key) ?? await AddCollection();
		}
		...
		{
			var produtoCollection = await this.produtoService.GetItems();
